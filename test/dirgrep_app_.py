# -*- coding: utf-8 -*-
"""
DirStringSearch v2.5.6（セル青背景 / 依存なし / WinAPIクリップボード / クリック素通し）
- 初回のセル選択から必ず青背景が出るよう、mousedown時に overlay を show/lift + geometry 同期して即描画
- その後に after_idle でも再描画（bbox未確定ケースの保険）
- 既存仕様：セル青背景のみ、WinAPIコピー、複数選択→セルクリックで1セルに縮退、フォーカス外れでオーバーレイ隠す
"""
...
def set_clipboard_text_windows(text: str):
    ...
    try:
        ...
        if not user32.SetClipboardData(CF_UNICODETEXT, hglb):
            kernel32.GlobalFree(hglb)
...
class App(tk.Tk):
    ...
    def pos_to_cell(self, x, y):
        ...
        try:
            ...
            return None, None
        try:
            row_index = self.iids.index(row_id)
        except ValueError:
            return None, None
        return row_index, col_index

    def _ensure_overlay_ready(self):
        # mousedown直後にオーバーレイを完全に前面化し、幾何/レイアウト確定
        try:
    ...
    def select_all_cells(self, event=None):
        if not self.rows:
            return "break"
        self.anchor = (0, 0)
        self.sel_rect = (0, 0, len(self.rows)-1, len(self.COLS)-1)
        self.redraw_selection()
        self.after_idle(self.redraw_selection)
        return "break"

    def current_selection_cells(self):
        cells = []
        if not self.sel_rect:
            return cells
        self.update_idletasks()
        r1, c1, r2, c2 = self.sel_rect
        r1 = max(0, r1); c1 = max(0, c1); r2 = min(len(self.rows)-1, r2); c2 = min(len(self.COLS)-1, c2)
        for ri in range(r1, r2+1):
            iid = self.iids[ri] if ri < len(self.iids) else None
            if not iid:
                continue
            for ci in range(c1, c2+1):
                col = f"#{ci+1}"
                bbox = self.tree.bbox(iid, col)
                if not bbox or bbox[2] <= 2 or bbox[3] <= 2:
                    continue
                x, y, w, h = bbox
                text = ""
                if ri < len(self.rows) and ci < len(self.COLS):
                    text = str(self.rows[ri][ci])
                anc = self.tree.column(self.COLS[ci], "anchor")
                cells.append({"x1": x, "y1": y, "x2": x+w, "y2": y+h, "text": text, "anchor": anc})
        return cells

    def redraw_selection(self):
        self.overlay.set_cells(self.current_selection_cells())
    ...
    def build_tsv_from_selection(self):
        ...
        for ri in range(r1, r2+1):
            row = self.rows[ri]
            vals = [str(row[ci]) if ci < len(row) else "" for ci in range(c1, c2+1)]
